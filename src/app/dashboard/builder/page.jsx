...
import { Building, MapPin, TrendingUp, ShieldCheck, Layers, ArrowUpRight, Compass, CreditCard, FileText, CheckCircle2, X } from "lucide-react";
export default function BuilderDashboard() {
    const router = useRouter();
    const [user, setUser] = useState(null);
    const [matchingLands, setMatchingLands] = useState([]);
...
`;
    ...
    if (!user)
        return null;
    const stats = [
        { label: "Matching Vetted Lots", value: matchingLands.length.toString(), icon: Layers, color: "text-indigo-650 bg-indigo-50 border-indigo-200" },
        { label: "Zoning Audits", value: "8 Completed", icon: ShieldCheck, color: "text-emerald-700 bg-emerald-50 border-emerald-250" },
        { label: "Active Site Demands", value: "3 Listed", icon: Building, color: "text-amber-705 bg-amber-50 border-amber-250" },
    ];
    return (<div className="min-h-screen bg-transparent text-slate-805 flex flex-col font-sans">
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-205 pb-3">
              <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                <Building className="h-4.5 w-4.5 text-indigo-650"/>
                Recommended Vetted Lots ({matchingLands.length})
              </h3>
              <Link href="/listings" className="text-xs font-bold text-indigo-605 hover:text-indigo-800">
                View all listings
              </Link>
            </div>
            ...
            {loading ? (<div className="space-y-3">
              ...
              </div>) : matchingLands.length === 0 ? (<div className="text-center py-12 bg-white/50 rounded-3xl border border-slate-200 p-6 shadow-2xl">
                <p className="text-sm font-semibold text-slate-650 font-sans">No matching plots found in registry.</p>
              </div>) : (<div className="space-y-4">
                {matchingLands.map((prop) => (<div key={prop.id} className="bg-white/85 border border-slate-200 rounded-3xl p-5 shadow-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:shadow-2xl transition-all duration-300">
                    <div className="flex items-center gap-4">
                      <div className="h-16 w-16 rounded-2xl overflow-hidden bg-slate-205 shrink-0">
                        <img src={prop.image && prop.image.startsWith("http") ? prop.image : "https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=100"} alt="" className="h-full w-full object-cover"/>
                      </div>
                      <div>
                    ...
                    <Link href="/listings" className="shrink-0">
                      <button className="flex items-center gap-1.5 px-4 py-2 border border-slate-200 hover:bg-slate-100 bg-white text-slate-700 rounded-xl text-xs font-bold transition-all cursor-pointer">
                        View Details
          ...
          <div className="space-y-6 lg:col-span-1">
            ...
            {!user.verified && (
              <div className="bg-gradient-to-br from-indigo-900/60 to-slate-900/80 border border-indigo-500/30 rounded-3xl p-5 shadow-2xl space-y-3.5 relative overflow-hidden">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-xs font-black text-white uppercase tracking-wider">Builder Verification</h4>
                    <p className="text-[10px] text-slate-300 font-semibold mt-1 leading-normal">
                      Verify your builder license, construction record, and corporate registration to clear verification checks.
                    </p>
                  </div>
                </div>
            ...
            <div className="bg-white/80 border border-slate-202 rounded-3xl p-5 shadow-2xl space-y-4">
              <h4 className="text-sm font-bold text-slate-900 border-b border-slate-200 pb-3">Developer Tools</h4>
              <div className="grid grid-cols-1 gap-2.5">
                <Link href="/ai-price">
                  <div className="flex items-center gap-3 p-3 border border-slate-200 rounded-xl hover:bg-slate-50 bg-white cursor-pointer">
                    ...
                    <span className="text-xs font-bold text-slate-700">Valuation Modifiers Estimator</span>
                  </div>
                </Link>
                <div onClick={() => { if(!user.verified) { setShowVerifyModal(true); setVerifyStep("form"); } }} className="flex items-center gap-3 p-3 border border-slate-200 rounded-xl hover:bg-slate-50 bg-white cursor-pointer">
                  ...
                  <span className="text-xs font-bold text-slate-705">{user.verified ? "Builder Vetted (Active)" : "Verify Business Entity"}</span>
                </div>
              </div>
            </div>
            ...
            {/* Site Requirements */}
            <div className="bg-white/80 border border-slate-202 rounded-3xl p-5 shadow-2xl space-y-4">
              <h4 className="text-sm font-bold text-slate-900 border-b border-slate-200 pb-3">Active Site Requirements</h4>
        ...
        ].map((req, idx) => (<div key={idx} className="space-y-1">
                    <div className="flex justify-between items-center text-indigo-600 uppercase tracking-wide text-[10px]">
                      <span>{req.region}</span>
                    </div>
                    <p className="text-[11px] text-slate-655 font-medium leading-relaxed bg-slate-50 border border-slate-200 p-2.5 rounded-xl">
                      {req.demand}
                    </p>
                  </div>))}
              </div>
