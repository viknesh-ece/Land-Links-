...
import { Plus, MapPin, MessageSquare, ShieldCheck, TrendingUp, Layers, ArrowUpRight } from "lucide-react";
export default function LandownerDashboard() {
        ...
        async function fetchMyProperties() {
            try {
                ...
                if (res.ok) {
                    ...
                    // For landowners, we can simulate that they created some properties
                    // e.g. every odd property ID belongs to this active landowner mock
    ...
    if (!user)
        return null;
    const stats = [
        { label: "Listed Parcels", value: myProperties.length.toString(), icon: Layers, color: "text-indigo-400 bg-indigo-950/40 border-indigo-900/50" },
        { label: "Developer Leads", value: "14 Inquiries", icon: MessageSquare, color: "text-emerald-400 bg-emerald-950/40 border-emerald-900/50" },
        { label: "Deed Title Status", value: "Fully Vetted", icon: ShieldCheck, color: "text-amber-400 bg-amber-950/40 border-amber-900/50" },
    ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {stats.map((stat, idx) => {
            ...
            return (<div key={idx} className="bg-[#090d16]/75 border border-slate-800/80 p-6 rounded-3xl shadow-2xl flex items-center gap-4">
                ...
                <div>
                  <p className="text-xs font-bold text-slate-450 uppercase tracking-wider">{stat.label}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {loading ? (<div className="space-y-3">
              ...
              </div>) : myProperties.length === 0 ? (<div className="text-center py-12 bg-[#090d16]/50 rounded-3xl border border-slate-800/80 p-6 shadow-2xl">
                <p className="text-sm font-semibold text-slate-400">You haven't listed any properties yet.</p>
                <Link href="/listings/create" className="mt-3 inline-flex items-center text-xs font-bold text-indigo-400 hover:text-indigo-300">
                  Submit listing wizard <ArrowUpRight className="h-3 w-3"/>
                </Link>
              </div>) : (<div className="space-y-4">
                {myProperties.map((prop) => (<div key={prop.id} className="bg-[#090d16]/75 border border-slate-800 rounded-3xl p-5 shadow-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:shadow-2xl transition-all duration-300">
                    ...
                    <Link href="/listings" className="shrink-0">
                      <button className="flex items-center gap-1 px-4 py-2 border border-slate-800 hover:bg-slate-900 bg-slate-955/40 text-slate-305 rounded-xl text-xs font-bold transition-all cursor-pointer">
                        Manage listing
          ...
          <div className="space-y-6 lg:col-span-1">
            ...
            <div className="bg-[#090d16]/75 border border-slate-800 rounded-3xl p-5 shadow-2xl space-y-4">
              <h4 className="text-sm font-bold text-white border-b border-slate-800 pb-3">Quick Actions</h4>
              <div className="grid grid-cols-1 gap-2.5">
                <Link href="/ai-price">
                  <div className="flex items-center gap-3 p-3 border border-slate-850 rounded-xl hover:bg-slate-900 bg-slate-955/40 cursor-pointer">
                    ...
                    <span className="text-xs font-bold text-slate-300">Calculate AI Valuation</span>
                  </div>
                </Link>
                <div className="flex items-center gap-3 p-3 border border-slate-850 rounded-xl hover:bg-slate-900 bg-slate-955/40 cursor-pointer">
                  ...
                  <span className="text-xs font-bold text-slate-300">Verify Deed Documents</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-[#090d16]/75 border border-slate-800 rounded-3xl p-5 shadow-2xl space-y-4">
              <h4 className="text-sm font-bold text-white border-b border-slate-800 pb-3">Recent Developer Inquiries</h4>
            ...
            { sender: "Pooja Mehta (Capital Fund)", time: "1 day ago", snippet: "Would you be open to a joint venture agreement or only direct acquisition?" }
                    ...
                    <p className="text-[11px] text-slate-400 font-medium leading-relaxed bg-slate-950 border border-slate-850 p-2.5 rounded-xl">
                      {msg.snippet}
                    </p>
                  </div>))}
              </div>
