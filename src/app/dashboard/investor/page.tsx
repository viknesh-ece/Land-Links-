...
export default function InvestorDashboard() {
  ...
  const [vettedDeals, setVettedDeals] = useState<any[]>([]);
  ...
  const [activeSyn, setActiveSyn] = useState<any>(null);
  const [fundAmount, setFundAmount] = useState<number>(0);
  const [fundState, setFundState] = useState<"idle" | "processing" | "success">("idle");
  ...
  const handleConfirmFund = () => {
    if (fundAmount < activeSyn.minInvestment) return;
                  ...
                  <div key={prop.id} className="bg-[#090d16]/75 border border-slate-800 rounded-3xl p-5 shadow-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:shadow-2xl transition-all duration-300">
                    <div className="flex items-center gap-4">
                      <div className="h-16 w-16 rounded-2xl overflow-hidden bg-slate-800 shrink-0">
                        <img 
                          src={prop.image && prop.image.startsWith("http") ? prop.image : "https://images.unsplash.com/photo-1502082553048-f009c37129b9?w=100"} 
                          alt="" 
            ...
            <div className="bg-[#090d16]/75 border border-slate-800 rounded-3xl p-5 shadow-2xl space-y-4">
              ...
              <div className="space-y-4 text-xs font-bold">
                {syndicates.map((syn) => {
                  ...
                  return (
                    <div key={syn.id} className="border border-slate-850 bg-slate-955/30 p-4 rounded-2xl space-y-3">
      ...
      {activeSyn && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-[#090d16] border border-slate-800 p-6 sm:p-8 rounded-3xl max-w-md w-full shadow-2xl relative space-y-6 animate-in zoom-in-95 duration-200">
            {/* Close Button */}
            <button
              onClick={() => fundState !== "processing" && setActiveSyn(null)}
              disabled={fundState === "processing"}
              className="absolute right-4 top-4 text-slate-400 hover:text-white disabled:opacity-30 cursor-pointer transition-all border-0 bg-transparent"
            >
              <X className="h-5 w-5" />
            </button>
              ...
              <>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      type="number"
                      min={activeSyn.minInvestment}
                      step={100000}
                      value={fundAmount}
                      onChange={(e) => setFundAmount(Number(e.target.value))}
                      className="w-full px-4 py-3 rounded-xl bg-slate-955 border border-slate-850 focus:outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-500 text-sm font-semibold text-white placeholder:text-slate-600"
                    />
                ...
                {fundAmount < activeSyn.minInvestment && (
                  <p className="text-xs font-bold text-rose-455 text-center bg-rose-955/20 border border-rose-900/40 p-2.5 rounded-xl">
                ...
                <button
                  ...
                  disabled={fundAmount < activeSyn.minInvestment}
                  className="w-full py-3.5 bg-indigo-650 hover:bg-indigo-600 disabled:bg-slate-800 disabled:text-slate-500 text-white rounded-2xl font-bold text-sm shadow-md transition-all active:scale-[0.98] cursor-pointer text-center border-0"
                >
                  ...
                  {[
                    ...
                    return (
                      <div key={idx} className="flex items-center gap-2.5 text-xs font-bold">
                        {isDone ? (
                          <div className="h-4.5 w-4.5 rounded-full bg-emerald-950 border border-emerald-900 text-emerald-400 flex items-center justify-center shrink-0">
                            <Check className="h-3 w-3" />
                          </div>
                        ) : isCurrent ? (
                          <div className="h-4.5 w-4.5 rounded-full bg-indigo-950 border border-indigo-900 text-indigo-400 flex items-center justify-center shrink-0">
                            <div className="h-1.5 w-1.5 bg-indigo-400 rounded-full animate-ping"></div>
                          </div>
                        ) : (
                          <div className="h-4.5 w-4.5 rounded-full bg-slate-900 border border-slate-800 text-slate-600 flex items-center justify-center shrink-0">
            ...
            {fundState === "success" && (
              <div className="py-6 flex flex-col items-center justify-center text-center space-y-5 animate-in fade-in duration-300">
                ...
                <button
                  onClick={() => setActiveSyn(null)}
                  className="w-full py-3 bg-slate-950 hover:bg-slate-900 border border-slate-850 hover:border-slate-700 text-slate-350 hover:text-white rounded-xl text-xs font-bold transition-all cursor-pointer text-center"
                >
