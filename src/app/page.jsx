...
export default function Home() {
    ...
    if (!authChecked) {
        return (
          <div className="min-h-screen bg-[#f0fdf4] text-emerald-950 flex items-center justify-center p-4 font-sans">
            <div className="text-center space-y-4">
              <div className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-700 border border-emerald-300 animate-pulse">
    ...
    return (
    <div className="min-h-screen text-emerald-950 font-sans selection:bg-emerald-600 selection:text-white pb-16">
      <Navbar />
      ...
      <section className="relative overflow-hidden pt-16 pb-20 md:pt-24 md:pb-28">
        ...
        <div className="absolute top-0 left-1/4 w-[500px] h-[500px] bg-emerald-200/50 rounded-full blur-3xl pointer-events-none"></div>
        <div className="absolute bottom-10 right-10 w-[450px] h-[450px] bg-teal-200/40 rounded-full blur-3xl pointer-events-none"></div>
        
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center max-w-4xl mx-auto">
            
            {/* Tagline Badge */}
            <div className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-emerald-100/90 border border-emerald-300 text-emerald-900 text-xs font-black uppercase tracking-wider mb-6 shadow-sm">
              <Sparkles className="h-4 w-4 text-emerald-700 animate-spin" style={{ animationDuration: "3s" }}/>
              <span>{t.hero?.badge || "Tamil Nadu Zero-Trust Land Marketplace"}</span>
            </div>

            {/* Main Headline */}
            <h1 className="text-3xl sm:text-5xl md:text-6xl font-black tracking-tight text-emerald-950 leading-[1.15] mb-6">
              {lang === "ta" ? (
                t.hero?.title
              ) : (
                <>
            ...
            {/* Subtitle */}
            <p className="text-base sm:text-lg text-emerald-900/80 font-medium max-w-3xl mx-auto mb-10 leading-relaxed">
            ...
            {/* Luxury Green & White Search Box */}
            <div className="bg-white border border-emerald-200 p-3 rounded-2xl shadow-xl shadow-emerald-900/5 max-w-3xl mx-auto mb-12 flex flex-col md:flex-row gap-2.5">
              <div className="flex-1 relative flex items-center">
                <Search className="h-4.5 w-4.5 text-emerald-600 left-3 absolute"/>
                <input 
                  type="text" 
                  placeholder={t.actions?.searchPlaceholder || "Search by district, survey no, or keyword..."}
                  value={searchQuery} 
                  onChange={(e) => setSearchQuery(e.target.value)} 
                  className="w-full pl-9 pr-3 py-2.5 text-sm bg-emerald-50/50 border border-emerald-200 rounded-xl focus:bg-white focus:outline-none focus:border-emerald-600 font-medium text-emerald-950 placeholder:text-emerald-700/60"
                />
              </div>
              <div className="h-px md:h-8 w-full md:w-px bg-emerald-200 self-center"></div>
              <div className="flex-1 relative flex items-center">
                <MapPin className="h-4.5 w-4.5 text-teal-600 left-3 absolute"/>
                <input 
                  type="text" 
              ...
              <Link 
                ...
                className="bg-gradient-to-r from-emerald-600 to-teal-700 hover:from-emerald-700 hover:to-teal-800 text-white font-black text-sm px-6 py-3 rounded-xl shadow-md shadow-emerald-600/25 active:scale-[0.98] transition-all flex items-center justify-center gap-2 whitespace-nowrap cursor-pointer border-0"
              >
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href="/spatial-studio" className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-8 py-4 rounded-2xl text-base font-black bg-gradient-to-r from-emerald-600 to-teal-700 hover:from-emerald-700 hover:to-teal-800 text-white shadow-lg shadow-emerald-600/25 hover:scale-[1.02] transition-all duration-200">
                <span>{lang === "ta" ? "3D நில வரைபடத்தை தொடங்கு" : "Launch 3D Spatial Studio"}</span>
                <Sparkles className="h-5 w-5 text-emerald-200 animate-spin" style={{ animationDuration: "5s" }}/>
              </Link>
              <Link href="/listings/create" className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-8 py-4 rounded-2xl text-base font-bold bg-white hover:bg-emerald-50 text-emerald-950 border border-emerald-300 shadow-md hover:scale-[1.02] transition-all duration-200">
      ...
      <section className="relative -mt-8 pb-16 z-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="bg-white rounded-3xl border border-emerald-100 p-8 md:p-12 shadow-xl shadow-emerald-900/5">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12 divide-y md:divide-y-0 lg:divide-x divide-emerald-100">
              {stats.map((stat, idx) => (
                <div key={idx} className="pt-6 first:pt-0 md:pt-0 lg:px-6 text-center md:text-left transition-all duration-300 hover:scale-[1.03] cursor-default group">
                  <p className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-800 tracking-tight">{stat.value}</p>
                  <p className="text-sm font-bold text-emerald-950 mt-2">{stat.label}</p>
                  <p className="text-xs text-emerald-800/70 font-medium mt-1 leading-normal">{stat.desc}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row items-start md:items-end justify-between mb-12">
            <div>
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-100 border border-emerald-300 text-emerald-900 text-xs font-black uppercase tracking-wider mb-3">
              ...
              <p className="text-emerald-800/80 font-medium mt-2 max-w-xl text-sm">
            ...
            <Link href="/listings" className="mt-4 md:mt-0 inline-flex items-center gap-1 text-sm font-bold text-emerald-700 hover:text-emerald-800 hover:gap-2 transition-all">
          ...
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[1, 2, 3].map((num) => (
                <div key={num} className="bg-white rounded-2xl border border-emerald-100 p-5 shadow-sm space-y-4 animate-pulse">
                  <div className="h-48 w-full bg-emerald-50 rounded-xl"></div>
                  <div className="h-6 w-1/3 bg-emerald-50 rounded"></div>
                  <div className="h-4 w-3/4 bg-emerald-50 rounded"></div>
                  <div className="h-10 w-full bg-emerald-50 rounded-xl"></div>
                </div>
              ))}
            </div>
          ) : properties.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-3xl border border-emerald-100 p-8 shadow-sm">
              ...
              <h3 className="text-lg font-bold text-emerald-950">No Listings in Database Yet</h3>
              <p className="text-emerald-800/80 text-sm max-w-md mx-auto mt-1">
                Be the first to list your land on India's premier digital platform.
              </p>
              <Link href="/listings/create" className="mt-5 inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-700 hover:from-emerald-700 hover:to-teal-800 text-white font-bold text-sm shadow-md transition-all">
                Add Your Property
              </Link>
            </div>
          ) : (
      ...
      <section className="py-20 border-y border-emerald-100 bg-emerald-50/40">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-3xl sm:text-4xl font-black text-emerald-950 tracking-tight">
              {lang === "ta" ? "அனைத்து பயனர் பணியிடங்கள்" : "Tailored Ecosystem Workspaces"}
            </h2>
            <p className="text-emerald-800/80 font-medium mt-4 text-sm">
              {lang === "ta"
            ...
            {roles.map((role, idx) => {
              ...
              return (
                <ThreeDTilt key={idx} className="flex flex-col h-full">
                  <div className="flex flex-col bg-white border border-emerald-100 rounded-3xl p-8 hover:border-emerald-400 hover:shadow-xl hover:shadow-emerald-900/10 transition-all duration-300 h-full shadow-sm">
                    <div className={`inline-flex h-12 w-12 items-center justify-center rounded-2xl border ${role.color} mb-6 shadow-sm`}>
                      <Icon className="h-6 w-6"/>
                    </div>
                    <h3 className="text-2xl font-black text-emerald-950 mb-3">{role.title}</h3>
                    <p className="text-emerald-900/80 text-sm leading-relaxed mb-8 flex-grow">{role.description}</p>
                    
                    <ul className="space-y-3.5 mb-8">
                      {role.features.map((feat, fidx) => (
                        <li key={fidx} className="flex items-center gap-2.5 text-xs text-emerald-950 font-bold">
                    ...
                    <Link href={role.link} className="flex items-center justify-center gap-2 w-full py-3.5 rounded-xl border border-emerald-200 bg-emerald-50 hover:bg-emerald-100 text-emerald-900 font-black text-xs transition-all duration-200 shadow-sm uppercase tracking-wider">
      ...
      <section className="py-20 relative overflow-hidden">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            <div>
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-100 border border-emerald-300 text-emerald-900 text-xs font-bold uppercase tracking-wider mb-6">
              ...
              <h2 className="text-3xl sm:text-4xl font-black tracking-tight text-emerald-950 mb-6">
                {t.homeSections?.valuationTitle || "Instant Automated Land Valuation"}
              </h2>
              <p className="text-emerald-900/80 font-medium leading-relaxed mb-8 text-sm">
                ...
                {features.map((feat, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex gap-4">
                      ...
                      <div>
                        <h4 className="text-base font-bold text-emerald-950">{feat.title}</h4>
                        <p className="text-xs text-emerald-800/70 font-medium mt-1 leading-relaxed">{feat.description}</p>
                      </div>
                    </div>
                  );
                })}
            ...
            <ThreeDTilt>
              <div className="bg-white border border-emerald-200 rounded-3xl p-6 sm:p-8 shadow-xl shadow-emerald-900/5 relative">
                <div className="flex justify-between items-center border-b border-emerald-100 pb-4 mb-6">
                  ...
                  <span className="text-xs font-bold text-emerald-900 bg-emerald-100 border border-emerald-300 px-2 py-0.5 rounded">Confidence: 96.4%</span>
                </div>
                ...
                <form onSubmit={handleQuickCalculate} className="space-y-5">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-emerald-50/40 border border-emerald-200 rounded-xl px-4 py-3 text-sm focus:bg-white focus:outline-none focus:border-emerald-600 text-emerald-950 placeholder:text-emerald-700/60 font-medium"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <select 
                      ...
                      className="w-full bg-emerald-50/40 border border-emerald-200 rounded-xl px-4 py-3 text-sm focus:bg-white focus:outline-none focus:border-emerald-600 text-emerald-950 font-semibold cursor-pointer"
                    >
                      <option value="Residential" className="text-emerald-950 bg-white">{lang === "ta" ? "குடியிருப்பு மண்டலம்" : "Residential Development"}</option>
                      <option value="Commercial" className="text-emerald-950 bg-white">{lang === "ta" ? "வணிக / ஐடி பூங்கா" : "Commercial / IT Park"}</option>
                      <option value="Industrial" className="text-emerald-950 bg-white">{lang === "ta" ? "தொழில்துறை மண்டலம்" : "Industrial Zone"}</option>
                      <option value="Agricultural" className="text-emerald-950 bg-white">{lang === "ta" ? "விவசாய நிலம்" : "Agricultural Land"}</option>
                    </select>
                  </div>

                  <button 
                    type="submit" 
                    className="w-full bg-gradient-to-r from-emerald-600 to-teal-700 hover:from-emerald-700 hover:to-teal-800 text-white font-black text-xs py-3.5 px-4 rounded-xl shadow-md shadow-emerald-600/25 active:scale-[0.98] transition-all flex items-center justify-center gap-2 cursor-pointer border-0 uppercase tracking-wider"
                  >
                ...
                {quickValuation !== null && (
                  <div className="mt-6 p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-center animate-in fade-in zoom-in-95 duration-200">
                    <p className="text-xs font-bold text-emerald-700 uppercase tracking-wider">{t.homeSections?.estimatedValue || "Estimated Base Value"}</p>
                    ...
                    <p className="text-[10px] text-emerald-700 mt-1">*Based on regional registrar baseline indices.</p>
                    <Link href="/ai-price" className="mt-3.5 inline-flex items-center gap-1.5 text-xs font-bold text-emerald-700 hover:text-emerald-900 transition-colors">
      ...
      <section className="py-20 bg-emerald-50/40">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            ...
            <p className="text-emerald-800/80 font-medium mt-3 text-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            {testimonials.map((test, idx) => (
              <div key={idx} className="bg-white border border-emerald-100 rounded-3xl p-8 shadow-sm hover:shadow-md transition-all flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-emerald-950/90 text-sm font-medium italic leading-relaxed">
                    "{test.quote}"
                  </p>
                </div>
                <div className="mt-6 pt-4 border-t border-emerald-100 flex items-center justify-between">
                  <div>
                    <h5 className="text-sm font-bold text-emerald-950">{test.author}</h5>
      ...
      {/* Footer */}
      <footer className="bg-emerald-950 text-emerald-100 py-16 border-t border-emerald-900 relative">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 border-b border-emerald-900 pb-12 mb-12">
            <div>
              <div className="flex items-center gap-2 mb-4">
                <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-bold">
                  <MapPin className="h-4.5 w-4.5"/>
                </div>
                <span className="text-xl font-black text-white tracking-wide">{t.brandName || "LandLinkX"}</span>
              </div>
              <p className="text-emerald-300/80 text-xs max-w-sm font-medium leading-relaxed">
            ...
            <div className="flex flex-wrap gap-8 lg:gap-16 text-xs font-bold">
              <div>
                ...
                <div className="flex flex-col gap-2.5 text-emerald-300/80">
                  <Link href="/listings" className="hover:text-emerald-300 transition-colors">{t.navListings || "Listings Directory"}</Link>
                  <Link href="/ai-price" className="hover:text-emerald-300 transition-colors">{t.navValuation || "AI Price Predictor"}</Link>
                  <Link href="/signup" className="hover:text-emerald-300 transition-colors">{t.actions?.publishListing || "Register Asset"}</Link>
                </div>
              </div>
              <div>
                <h6 className="text-white text-xs font-black uppercase tracking-wider mb-4">{lang === "ta" ? "பயனர் பகுதி" : "User Portal"}</h6>
                <div className="flex flex-col gap-2.5 text-emerald-300/80">
                  <Link href="/login" className="hover:text-emerald-300 transition-colors">{t.navLogin || "Sign In"}</Link>
                  <Link href="/signup" className="hover:text-emerald-300 transition-colors">{t.navSignup || "Create Account"}</Link>
                  <Link href="/dashboard" className="hover:text-emerald-300 transition-colors">{lang === "ta" ? "டாஷ்போர்டு" : "Portals Dashboard"}</Link>
                </div>
              </div>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row items-center justify-between text-xs font-medium text-emerald-400">
            <p>&copy; {new Date().getFullYear()} LandLinkX Inc. {lang === "ta" ? "அனைத்து உரிமைகளும் பாதுகாக்கப்பட்டவை." : "All rights reserved."}</p>
            <p className="mt-2 sm:mt-0 hover:text-emerald-200 transition-colors cursor-pointer">
