...
export default function Home() {
    ...
    return (<div className="min-h-screen bg-transparent text-slate-100 font-sans selection:bg-indigo-500 selection:text-white">
      <Navbar />
          ...
          <div className="text-center max-w-4xl mx-auto">
            ...
            <div className="bg-[#090d16]/80 backdrop-blur-xl border border-slate-800 p-3 rounded-2xl shadow-xl max-w-3xl mx-auto mb-12 flex flex-col md:flex-row gap-2.5 animate-scale-in opacity-0 delay-200">
              <div className="flex-1 relative flex items-center">
                ...
                <input type="text" placeholder="Keyword (e.g. industrial, IT park, residential)..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-3 py-2 text-sm bg-transparent border-0 focus:ring-0 focus:outline-none font-medium text-white placeholder:text-slate-500"/>
              </div>
              ...
              <div className="flex-1 relative flex items-center">
                ...
                <input type="text" placeholder="Location / Region..." value={searchLocation} onChange={(e) => setSearchLocation(e.target.value)} className="w-full pl-9 pr-3 py-2 text-sm bg-transparent border-0 focus:ring-0 focus:outline-none font-medium text-white placeholder:text-slate-500"/>
              </div>
              <Link href={`/listings?search=${encodeURIComponent(searchQuery + " " + searchLocation)}`} className="bg-indigo-650 hover:bg-indigo-600 text-white font-bold text-sm px-6 py-3 rounded-xl shadow-md shadow-indigo-900/20 active:scale-[0.98] transition-all flex items-center justify-center gap-2 whitespace-nowrap cursor-pointer">
                Search Listings
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 animate-slide-up opacity-0 delay-300">
              <Link href="/listings" className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-8 py-4 rounded-xl text-base font-bold bg-indigo-600 hover:bg-indigo-500 text-white shadow-xl shadow-indigo-900/20 hover:shadow-indigo-900/35 hover:scale-[1.01] transition-all duration-200">
                Browse Land Parcels
                <ArrowRight className="h-5 w-5"/>
              </Link>
              <Link href="/ai-price" className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-8 py-4 rounded-xl text-base font-bold bg-slate-900 hover:bg-slate-850 text-white shadow-lg hover:scale-[1.01] transition-all duration-200">
                Calculate AI Price
      ...
      <section className="py-20 bg-black">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row items-start md:items-end justify-between mb-12">
            ...
            <Link href="/listings" className="mt-4 md:mt-0 inline-flex items-center gap-1 text-sm font-bold text-indigo-400 hover:text-indigo-355 hover:gap-2 transition-all">
          ...
          {loading ? (<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[1, 2, 3].map((num) => (<div key={num} className="bg-[#090d16] rounded-2xl border border-slate-800/80 p-5 shadow-sm space-y-4 animate-pulse">
            ...
            </div>) : properties.length === 0 ? (<div className="text-center py-16 bg-[#090d16]/40 rounded-3xl border border-slate-800/60 p-8 shadow-sm">
              ...
              <Link href="/listings/create" className="mt-5 inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-sm shadow-md transition-all">
                Add Your Property
              </Link>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            return (<ThreeDTilt key={idx} className="flex flex-col h-full">
                  <div className="flex flex-col bg-[#090d16]/30 border border-slate-800/60 rounded-3xl p-8 hover:shadow-2xl hover:bg-[#090d16]/80 transition-all duration-300 h-full">
                    ...
                    <ul className="space-y-3.5 mb-8">
                      {role.features.map((feat, fidx) => (<li key={fidx} className="flex items-center gap-2.5 text-sm text-slate-300 font-semibold">
                          <CheckCircle2 className="h-4.5 w-4.5 text-emerald-500 shrink-0"/>
                          <span>{feat}</span>
                        </li>))}
                    </ul>

                    <Link href={role.link} className="flex items-center justify-center gap-2 w-full py-3.5 rounded-xl border border-indigo-950/60 bg-indigo-950/20 hover:bg-indigo-600 text-indigo-300 hover:text-white font-bold text-sm transition-all duration-200 shadow-sm">
                      Enter Workspace
              ...
              <div className="bg-[#090d16]/80 backdrop-blur-md border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl relative">
                ...
                <form onSubmit={handleQuickCalculate} className="space-y-5">
                  <div>
                    ...
                    <input type="number" step="0.01" placeholder="e.g. 2.5" required value={quickAcres} onChange={(e) => setQuickAcres(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-indigo-500 text-white placeholder:text-slate-650 font-medium"/>
                  </div>

                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Zoning Classification</label>
                    <select value={quickZoning} onChange={(e) => setQuickZoning(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-indigo-500 text-white font-semibold cursor-pointer">
                  ...
                  <button type="submit" className="w-full bg-indigo-650 hover:bg-indigo-600 text-white font-bold text-sm py-3.5 px-4 rounded-xl shadow-lg active:scale-[0.98] transition-all flex items-center justify-center gap-2 cursor-pointer border-0">
                    <Sparkles className="h-4.5 w-4.5 text-amber-300"/>
                    Estimate Valuation
                  </button>
                </form>

                {quickValuation !== null && (<div className="mt-6 p-4 rounded-2xl bg-indigo-950/40 border border-indigo-900/50 text-center animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <Link href="/ai-price" className="mt-3.5 inline-flex items-center gap-1.5 text-xs font-bold text-indigo-300 hover:text-white transition-colors">
      ...
      <section className="py-20 bg-transparent">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            {testimonials.map((test, idx) => (<div key={idx} className="bg-[#090d16]/50 border border-slate-800/80 rounded-3xl p-8 shadow-2xl flex flex-col justify-between backdrop-blur-sm">
                <div>
