  ...
  <div class="receipt-container">
    ...
    return (<div className="min-h-screen bg-[#f0fdf4] text-emerald-950 flex flex-col font-sans">
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col h-[calc(100vh-80px)]">
        ...
        {/* dm workspace header */}
        <div className="flex items-center gap-2 border-b border-emerald-200 pb-4 mb-6 shrink-0">
          <MessageSquare className="h-6 w-6 text-emerald-600"/>
          <h1 className="text-2xl font-black text-emerald-950 tracking-tight">
            {t.inbox?.pageTitle || "Direct Negotiation Hub"}
          </h1>
          <span className="text-[10px] font-bold text-emerald-900 bg-emerald-100 border border-emerald-300 px-2.5 py-0.5 rounded-full ml-2">
            Secure Encrypted Bids
          </span>
        </div>

        {/* chat grid container */}
        <div className="flex-grow flex border border-emerald-100 bg-white overflow-hidden shadow-sm h-0 rounded-3xl">
          
          {/* threads sidebar */}
          <div className="w-80 border-r border-emerald-100 flex flex-col shrink-0">
            <div className="p-4 border-b border-emerald-50">
              <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-800">Active Conversations</p>
            </div>
            ...
            return (<button key={t.id} onClick={() => {
                    ...
                    : "border-transparent hover:bg-emerald-50/50 text-emerald-900/80 hover:text-emerald-950"}`}>
                    <div className="h-9 w-9 rounded-xl bg-emerald-50 border border-emerald-200 flex items-center justify-center shrink-0">
                      <User className={`h-4.5 w-4.5 ${isActive ? "text-emerald-600" : "text-emerald-500"}`}/>
                    </div>
                    <div className="flex-grow min-w-0">
                      <div className="flex items-center justify-between">
                        <p className="font-bold text-xs truncate flex items-center gap-1 text-emerald-950">
                          {t.name}
