      ...
      <main className="flex-grow flex items-center justify-center p-4 py-12 relative overflow-hidden">
        ...
        <div className="w-full max-w-2xl bg-[#090d16]/75 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 sm:p-10 shadow-2xl relative z-10">
          ...
          {submitted ? (<div className="text-center py-12 space-y-6 animate-in zoom-in-95 duration-200">
              ...
              <div className="mb-8">
                <div className="flex justify-between items-center mb-3">
                  ...
                  <span className="h-2 w-32 bg-slate-800 rounded-full overflow-hidden">
                    <span className="h-full bg-indigo-500 block transition-all duration-300" style={{ width: `${(step / 4) * 100}%` }}></span>
                  </span>
                </div>
              ...
              {error && (<div className="mb-6 p-4 rounded-2xl bg-rose-955/30 border border-rose-900/50 text-rose-400 text-xs font-bold flex gap-2">
              ...
              {step === 1 && (<div className="space-y-4 animate-scale-in">
                  <div>
                    ...
                    <input type="text" placeholder="e.g. 5 Acres Commercial land near IT corridor" value={form.title} className="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-850 focus:outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-500 text-sm font-semibold text-white placeholder:text-slate-600" onChange={(e) => setForm({ ...form, title: e.target.value })}/>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="text" placeholder="e.g. Whitefield, Bangalore" value={form.location} className="w-full pl-10 pr-4 py-3 rounded-xl bg-slate-950 border border-slate-850 focus:outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-500 text-sm font-semibold text-white placeholder:text-slate-600" onChange={(e) => setForm({ ...form, location: e.target.value })}/>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="number" placeholder="e.g. 45000000" value={form.price} className="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-850 focus:outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-500 text-sm font-semibold text-white placeholder:text-slate-600" onChange={(e) => setForm({ ...form, price: e.target.value })}/>
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">Land Size (Acres)</label>
                      <input type="number" step="0.01" placeholder="e.g. 2.5" value={form.acres} className="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-850 focus:outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-500 text-sm font-semibold text-white placeholder:text-slate-600" onChange={(e) => setForm({ ...form, acres: e.target.value })}/>
                    </div>
                  </div>

                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">Property Description</label>
                    <textarea placeholder="Detail soil depth, zoning modification history, nearby national highways, water resources..." rows={4} value={form.description} className="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-850 focus:outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-500 text-sm font-semibold text-white placeholder:text-slate-600" onChange={(e) => setForm({ ...form, description: e.target.value })}/>
                  </div>
                </div>)}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="number" placeholder="e.g. 12" value={form.roadWidth} className="w-full pl-10 pr-4 py-3 rounded-xl bg-slate-950 border border-slate-850 focus:outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-500 text-sm font-semibold text-white placeholder:text-slate-600" onChange={(e) => setForm({ ...form, roadWidth: e.target.value })}/>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <select value={form.waterSource} onChange={(e) => setForm({ ...form, waterSource: e.target.value })} className="w-full pl-10 pr-4 py-3 rounded-xl bg-slate-950 border border-slate-850 focus:outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-500 text-sm font-bold text-slate-200 cursor-pointer appearance-none">
              ...
              {step === 3 && (<div className="space-y-4 animate-scale-in">
                  <div className="flex justify-between items-center">
                    ...
                    <button type="button" onClick={() => setGisPoints([])} className="text-xs font-bold text-indigo-405 hover:text-indigo-300 cursor-pointer">
                      Clear Coordinates
                    </button>
                  </div>
                  
                  <div className="relative">
                    <svg viewBox="0 0 300 200" className="w-full h-56 rounded-2xl bg-slate-950 border border-slate-855 cursor-crosshair relative overflow-hidden" onClick={(e) => {
                    ...
                    setGisPoints(prev => [...prev, { x, y }]);
                      ...
                      {gisPoints.length > 0 && (<>
                          {gisPoints.length >= 3 && (<polygon points={gisPoints.map(p => `${p.x},${p.y}`).join(" ")} fill="rgba(99, 102, 241, 0.15)" stroke="none"/>)}
                        ...
                        return (<line key={idx} x1={prev.x} y1={prev.y} x2={p.x} y2={p.y} stroke="#6366f1" strokeWidth="2"/>);
                          ...
                          {gisPoints.map((p, idx) => (<g key={idx}>
                              ...
                              <text x={p.x + 8} y={p.y + 4} fill="#818cf8" fontSize="7" fontWeight="bold">
                                V{idx + 1} ({p.x}, {p.y})
                              </text>
                            </g>))}
                        </>)}
                  ...
                  <div className="bg-[#090d16] border border-slate-900 rounded-2xl p-4">
                    ...
                    {gisPoints.length > 0 && (<div className="mt-3 flex flex-wrap gap-2 text-[10px] font-bold text-slate-400">
                        {gisPoints.map((p, idx) => (<span key={idx} className="bg-slate-950 border border-slate-850 px-2.5 py-1 rounded-lg">
                            V{idx + 1}: {p.x}m, {p.y}m
                          </span>))}
                      </div>)}
                  </div>
              ...
              {step === 4 && (<div className="space-y-4 animate-scale-in">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">Property Image URL</label>
                      <input type="text" placeholder="Paste high-res image URL..." value={form.image} className="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-850 focus:outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-500 text-sm font-semibold text-white placeholder:text-slate-650" onChange={(e) => setForm({ ...form, image: e.target.value })}/>
                    </div>
                    <div>
                    ...
                    if (scanState === "idle") {
                        ...
                        const interval = setInterval(() => {
                            setScanProgress(prev => {
                                ...
                                if (nextVal === 25)
                                    setScanText("Extracting title records via OCR...");
                                if (nextVal === 50)
                                    setScanText("Validating land survey registers...");
                                if (nextVal === 75)
                                    setScanText("Checking municipal encumbrances...");
                                if (nextVal === 100)
                                    setScanText("Audit successful! Title details clear.");
                                return nextVal;
                            });
                        }, 800);
                    }
                }} className={`border-2 border-dashed rounded-3xl p-5 text-center cursor-pointer transition-all ${scanState === "completed"
                    ...
                    : "border-slate-800 hover:border-indigo-500 bg-slate-950/40"}`}>
                        ...
                        {scanState === "scanning" && (<div className="py-2 space-y-3">
                        ...
                        {scanState === "completed" && (<div className="py-1">
                  ...
                  {scanState === "completed" && (<div className="bg-[#090d16]/80 border border-slate-900 p-4 rounded-2xl grid grid-cols-3 gap-2.5 text-center animate-in zoom-in-95 duration-200">
                  ...
                  <div className="bg-indigo-950/20 border border-indigo-900/30 p-4 rounded-2xl flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <label className="flex items-center gap-2 mt-3 cursor-pointer text-xs font-bold text-indigo-400">
                        <input type="checkbox" checked={form.deedVerified} disabled={scanState !== "completed"} onChange={(e) => setForm(f => ({ ...f, deedVerified: e.target.checked }))} className="rounded text-indigo-500 focus:ring-indigo-550 border-slate-700 bg-slate-950 disabled:opacity-50"/>
              ...
              <div className="flex justify-between mt-10 pt-6 border-t border-slate-800">
                {step > 1 ? (<button type="button" onClick={prevStep} className="flex items-center gap-1.5 px-5 py-3 border border-slate-800 bg-slate-955/40 text-slate-305 hover:bg-slate-900 hover:border-slate-700 rounded-xl font-bold text-sm transition-all cursor-pointer">
                    <ArrowLeft className="h-4 w-4"/>
                    Back
                  </button>) : (<div></div>)}

                {step < 4 ? (<button type="button" onClick={nextStep} className="flex items-center gap-1.5 px-6 py-3 bg-indigo-650 hover:bg-indigo-600 text-white rounded-xl font-bold text-sm transition-all shadow-md active:scale-[0.98] cursor-pointer">
                    Continue
                    <ArrowRight className="h-4 w-4"/>
                  </button>) : (<button type="button" onClick={handleCreate} disabled={loading || !form.deedVerified} className="flex items-center gap-1.5 px-6 py-3 bg-indigo-650 hover:bg-indigo-600 disabled:bg-slate-800 disabled:text-slate-500 text-white rounded-xl font-bold text-sm transition-all shadow-md disabled:shadow-none active:scale-[0.98] cursor-pointer">
