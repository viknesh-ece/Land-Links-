    ...
    return (<main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-slate-800 pb-6 mb-8">
        ...
        <div className="flex items-center gap-3">
          <button onClick={fetchProperties} className="inline-flex h-11 w-11 items-center justify-center rounded-xl border border-slate-800 bg-[#090d16]/60 hover:bg-[#090d16] text-slate-300 transition-colors shadow-sm cursor-pointer" title="Refresh Listings">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="bg-[#090d16]/70 backdrop-blur-md rounded-2xl border border-slate-800/80 p-5 shadow-2xl space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="e.g. NH-48 highway, farm..." value={search} className="w-full pl-9 pr-3 py-2.5 rounded-xl bg-slate-950 border border-slate-850 focus:outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-505 text-xs font-semibold text-white placeholder:text-slate-500 transition-all" onChange={(e) => setSearch(e.target.value)}/>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <select value={locationFilter} onChange={(e) => setLocationFilter(e.target.value)} className="w-full pl-9 pr-3 py-2.5 rounded-xl bg-slate-950 border border-slate-850 focus:outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-505 text-xs font-bold text-slate-200 transition-all appearance-none cursor-pointer">
                  <option value="all" className="bg-slate-950 text-white">All Regions</option>
                  {uniqueLocations.map((loc) => (<option key={loc} value={loc} className="bg-slate-950 text-white">{loc}</option>))}
                </select>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <input type="number" placeholder="Min Price" value={minPrice} className="w-full px-3 py-2.5 rounded-xl bg-slate-950 border border-slate-850 focus:outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-505 text-xs font-semibold text-white placeholder:text-slate-500 transition-all" onChange={(e) => setMinPrice(e.target.value)}/>
                <input type="number" placeholder="Max Price" value={maxPrice} className="w-full px-3 py-2.5 rounded-xl bg-slate-950 border border-slate-850 focus:outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-505 text-xs font-semibold text-white placeholder:text-slate-500 transition-all" onChange={(e) => setMaxPrice(e.target.value)}/>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="number" placeholder="Min Acres" value={minAcres} className="w-full pl-9 pr-3 py-2.5 rounded-xl bg-slate-950 border border-slate-850 focus:outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-505 text-xs font-semibold text-white placeholder:text-slate-500 transition-all" onChange={(e) => setMinAcres(e.target.value)}/>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {["all", "Residential", "Commercial", "Industrial", "Agricultural"].map((zone) => (<label key={zone} className="flex items-center gap-2 text-xs font-bold text-slate-300 cursor-pointer">
                    <input type="radio" name="zoning" checked={zoningFilter === zone} onChange={() => setZoningFilter(zone)} className="rounded-full border-slate-700 bg-slate-950 text-indigo-500 focus:ring-indigo-550 h-3.5 w-3.5"/>
            ...
            <div className="pt-2 border-t border-slate-800">
              <label className="flex items-center justify-between text-xs font-bold text-slate-300 cursor-pointer">
                ...
                <input type="checkbox" checked={onlyVerified} onChange={(e) => setOnlyVerified(e.target.checked)} className="rounded border-slate-700 bg-slate-950 text-indigo-500 focus:ring-indigo-550 h-4.5 w-4.5 cursor-pointer"/>
              </label>
            </div>
        ...
        <div className="lg:col-span-3">
          {loading ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
              {[1, 2, 3, 4].map((num) => (<div key={num} className="bg-[#090d16] rounded-2xl border border-slate-800/80 p-5 shadow-sm space-y-4 animate-pulse">
            ...
            </div>) : filteredProperties.length === 0 ? (<div className="text-center py-20 bg-[#090d16]/50 border border-slate-800/80 rounded-3xl p-8 shadow-2xl">
