    ...
    return (
      <>
        <ThreeDTilt className="h-full">
          <div className="group bg-white rounded-2xl border border-emerald-100 hover:border-emerald-500 hover:shadow-xl hover:shadow-emerald-900/10 transition-all duration-300 flex flex-col h-full relative shadow-sm">
            <div className="relative h-52 w-full overflow-hidden bg-emerald-50">
              ...
              <div className="absolute top-3 left-3 inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-emerald-700 text-white text-[10px] font-black shadow-md uppercase tracking-wider">
              ...
              <button onClick={() => setIsSaved(!isSaved)} className={`absolute top-3 right-3 h-8.5 w-8.5 rounded-lg flex items-center justify-center border transition-all cursor-pointer ${isSaved
                  ...
                  : "bg-white/90 border-emerald-200 text-emerald-800 hover:text-rose-600 hover:bg-white"}`}>
                <Heart className={`h-4.5 w-4.5 ${isSaved ? "fill-current" : ""}`}/>
              </button>
            </div>

            <div className="p-5 flex flex-col flex-grow text-emerald-950">
              <div className="flex items-start justify-between gap-2 mb-2">
                <span className="text-xl font-black text-emerald-700">
                  ₹ {formattedPrice}
                </span>
                <span className="inline-flex items-center gap-1 text-[10px] font-extrabold text-emerald-900 bg-emerald-100/80 border border-emerald-300 px-2 py-0.5 rounded-md">
                  {lang === "ta" ? `நில எண் #${property.id}` : `Parcel #${property.id}`}
                </span>
              </div>

              <h3 className="text-base font-bold text-emerald-950 group-hover:text-emerald-700 transition-colors line-clamp-1 mb-2">
                {property.title}
              </h3>

              <div className="flex items-center gap-1 text-emerald-800/80 text-xs font-semibold mb-3">
                <MapPin className="h-3.5 w-3.5 text-emerald-600 shrink-0"/>
                <span className="line-clamp-1">{property.location}</span>
              </div>

              <p className="text-emerald-900/80 font-normal text-xs leading-relaxed line-clamp-2 mb-5 flex-grow">
                {property.description}
              </p>

              <div className="grid grid-cols-4 gap-2 pt-4 border-t border-emerald-100">
                <button onClick={() => setShowDetails(true)} className="col-span-2 flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-700 hover:from-emerald-700 hover:to-teal-800 text-white text-xs font-bold transition-all cursor-pointer border-0 shadow-md shadow-emerald-600/25">
                  <span>{lang === "ta" ? "விவரங்கள்" : "Details"}</span>
                  <ArrowUpRight className="h-3.5 w-3.5"/>
                </button>

                <button onClick={() => setShowContact(true)} className="flex items-center justify-center rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-800 hover:text-emerald-950 border border-emerald-200 text-xs font-semibold py-2 px-2 transition-all cursor-pointer" title="Contact Owner">
                  <Phone className="h-3.5 w-3.5"/>
                </button>

                <button onClick={() => setShowDeleteConfirm(true)} className="flex items-center justify-center rounded-xl bg-emerald-50 hover:bg-rose-50 text-emerald-800 hover:text-rose-600 border border-emerald-200 text-xs font-semibold py-2 px-2 transition-all cursor-pointer" title="Delete Listing">
